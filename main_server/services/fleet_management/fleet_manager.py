...
from main_server.infrastructure.database.repositories.mysql_robot_repository import (
...
class FleetManager:
    ...
    async def enable_obstacle_relay(self, robot_name: str):
        ...
        if not self.ai_processing_service:
            logger.error(
                "AIProcessingService가 설정되지 않아 장애물 릴레이를 시작할 수 없습니다."
            )
            return
        ...
        await self.ai_processing_service.start_obstacle_detection(
            robot_name, _relay_callback
        )
    ...
    async def enable_employee_relay(self, robot_name: str):
        ...
        if not self.ai_processing_service:
            logger.error(
                "AIProcessingService가 설정되지 않아 직원 인식 릴레이를 시작할 수 없습니다."
            )
            return
        ...
        await self.ai_processing_service.start_employee_verification(
            robot_name, _face_callback
        )
    ...
    async def restore_ai_relay_from_db(self):
        """
        메인서버 시작(재시작) 시 DB에 저장된 로봇 상태를 기반으로
        AI 추론 콜백을 재등록합니다.
        OFFLINE 로봇은 건너뜁니다.
        """
        if not self.ai_processing_service:
        ...
        try:
            ...
            for robot in robots:
                ...
                if robot.status in [
                    ...
                    await self.enable_obstacle_relay(robot.name)
                else:
                    await self.disable_obstacle_relay(robot.name)
                if robot.status in [RobotStatus.IDLE, RobotStatus.CHARGING]:
                    await self.enable_employee_relay(robot.name)
                else:
                    await self.disable_employee_relay(robot.name)
                logger.info(f"[{robot.name}] AI 릴레이 복원: status={robot.status}")
        except Exception as e:
            logger.error(f"AI 릴레이 복원 중 오류: {e}")
    ...
    async def update_forbidden_zones(self, zones: List[Dict]):
        ...
        try:
            ...
            logger.info(
                f"FleetManager: 금지 구역 {len(zones)}개 업데이트 및 {len(robots)}대 로봇 전파 완료."
            )
        except Exception as e:
    ...
    async def update_robot_status(
        ...
        if not robot:
            logger.warning(
                f"로봇 '{robot_id}'를 찾을 수 없어 상태 업데이트를 건너뜁니다."
            )
            return None
        ...
        if not has_changed:
            # 좌표 비교 (None 체크 포함)
            if (
                robot.pose_x is None
                or new_x is None
                or not math.isclose(robot.pose_x, new_x, abs_tol=1e-9)
            ):
                has_changed = True
            elif (
                robot.pose_y is None
                or new_y is None
                or not math.isclose(robot.pose_y, new_y, abs_tol=1e-9)
            ):
                has_changed = True
            elif (
                robot.battery_level is None
                or new_battery is None
                or not math.isclose(robot.battery_level, new_battery, abs_tol=1e-9)
            ):
                has_changed = True
